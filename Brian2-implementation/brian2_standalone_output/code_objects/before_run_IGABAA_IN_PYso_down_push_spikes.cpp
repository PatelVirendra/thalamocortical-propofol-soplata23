...
double* const _array_IGABAA_IN_PYso_delay_1 = _dynamic_array_IGABAA_IN_PYso_delay_1.empty()? 0 : &_dynamic_array_IGABAA_IN_PYso_delay_1[0];
const size_t _numdelay = _dynamic_array_IGABAA_IN_PYso_delay_1.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_IN_group__spikespace = _array_IN_group__spikespace;
    double*   _ptr_array_IGABAA_IN_PYso_delay_1 = _array_IGABAA_IN_PYso_delay_1;

    std::vector<double> &real_delays = _dynamic_array_IGABAA_IN_PYso_delay_1;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = IGABAA_IN_PYso_down.sources.empty() ? 0 : &(IGABAA_IN_PYso_down.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = IGABAA_IN_PYso_down.sources.size();
    IGABAA_IN_PYso_down.prepare(_n_sources,
                           _n_targets,
