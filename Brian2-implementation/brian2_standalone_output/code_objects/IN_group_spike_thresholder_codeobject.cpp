...
const size_t _numv = 20;
    ...
    double* __restrict  _ptr_array_IN_group_lastspike = _array_IN_group_lastspike;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const char not_refractory = _ptr_array_IN_group_not_refractory[_idx];
        const double v = _ptr_array_IN_group_v[_idx];
        char _cond;
        if(!not_refractory)
            _cond = ((v > _lio_1) && false) && false;
        else 
            _cond = ((v > _lio_1) && true) && true;

        if(_cond) {
            _ptr_array_IN_group__spikespace[_count++] = _idx;
            _ptr_array_IN_group_not_refractory[_idx] = false;
            _ptr_array_IN_group_lastspike[_idx] = _ptr_array_defaultclock_t[0];
        }
    }
    _ptr_array_IN_group__spikespace[N] = _count;
