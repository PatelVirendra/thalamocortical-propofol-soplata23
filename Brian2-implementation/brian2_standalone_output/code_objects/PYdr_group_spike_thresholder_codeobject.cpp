...
const size_t _numv = 100;
    ...
    double* __restrict  _ptr_array_PYdr_group_lastspike = _array_PYdr_group_lastspike;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const char not_refractory = _ptr_array_PYdr_group_not_refractory[_idx];
        const double v = _ptr_array_PYdr_group_v[_idx];
        char _cond;
        if(!not_refractory)
        ...
        if(_cond) {
            _ptr_array_PYdr_group__spikespace[_count++] = _idx;
            _ptr_array_PYdr_group_not_refractory[_idx] = false;
            _ptr_array_PYdr_group_lastspike[_idx] = _ptr_array_defaultclock_t[0];
        }
    }
    _ptr_array_PYdr_group__spikespace[N] = _count;
