...
double* const _array_IAMPA_PYso_IN_delay = _dynamic_array_IAMPA_PYso_IN_delay.empty()? 0 : &_dynamic_array_IAMPA_PYso_IN_delay[0];
const size_t _numdelay = _dynamic_array_IAMPA_PYso_IN_delay.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_PYso_group__spikespace = _array_PYso_group__spikespace;
    double*   _ptr_array_IAMPA_PYso_IN_delay = _array_IAMPA_PYso_IN_delay;

    std::vector<double> &real_delays = _dynamic_array_IAMPA_PYso_IN_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = IAMPA_PYso_IN_up.sources.empty() ? 0 : &(IAMPA_PYso_IN_up.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = IAMPA_PYso_IN_up.sources.size();
    IAMPA_PYso_IN_up.prepare(_n_sources,
                           _n_targets,
