...
void _before_run_INMDA_PYso_PYdr_down_push_spikes()
{
...
double* const _array_INMDA_PYso_PYdr_delay_1 = _dynamic_array_INMDA_PYso_PYdr_delay_1.empty()? 0 : &_dynamic_array_INMDA_PYso_PYdr_delay_1[0];
const size_t _numdelay = _dynamic_array_INMDA_PYso_PYdr_delay_1.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_PYso_group__spikespace = _array_PYso_group__spikespace;
    double*   _ptr_array_INMDA_PYso_PYdr_delay_1 = _array_INMDA_PYso_PYdr_delay_1;

    std::vector<double> &real_delays = _dynamic_array_INMDA_PYso_PYdr_delay_1;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = INMDA_PYso_PYdr_down.sources.empty() ? 0 : &(INMDA_PYso_PYdr_down.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = INMDA_PYso_PYdr_down.sources.size();
    INMDA_PYso_PYdr_down.prepare(_n_sources,
                           _n_targets,
